...
# 5b. Lifeboat: keep one tiny model cached as a guaranteed-fits offline fallback,
#     so a low-RAM machine always has *something* to fall back to (no internet
#     needed). Skip with SHTF_SKIP_LIFEBOAT=1.
LIFEBOAT="${repos[0]}"        # the smallest curated model
if [[ "${SHTF_SKIP_LIFEBOAT:-0}" != "1" && "$CHOSEN" != "$LIFEBOAT" ]]; then
